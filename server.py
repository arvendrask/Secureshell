#The link is the "Web" in WebSockets.
#  It's the bridge from HTTP to WebSockets.
#  In the link, details of the connection are negotiated, 
#  and either party can back out before completion if the terms are unfavorable.
#
#The link is the bridge from HTTP to Websockets, in the link terms of connection are confirmed.  
import time
import struct
import subprocess 
import socket
...
def validate_password(user, password):
    password = decrypt(password).decode("utf-8") #may be problem
...
# Calculating link response key
# When the server receives the link request, 
    ...
    def feed(self, data):
        ...
        if not self.linkn:
            ...
            if self.header.find('\r\n\r\n') != -1:
                ...
                self.header = parts[0]
                if self.dolink(self.header, parts[1]):
                    
                    logging.info("link successful")
            ...
            logging.debug("link is already complete")
...
# Read the next 64 bits and interpret those as an unsigned integer (The most significant bit MUST be 0). You're done.
    ...
    def dolink(self, header, key=None):
    
        logging.debug("Begin link: %s" % header)
            
        for line in header.split('\r\n')[1:]:
            name, value = line.split(': ', 1)
            
            if name.lower() == "sec-websocket-key":
                link =  make_link_response(value)

        logging.debug("Sending link %s" % link)
        self.client.send(link.encode())
        return True
